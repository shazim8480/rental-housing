...
import { useGetRegisteredUsersQuery } from "@/redux/feature/registered-users/registered-users-api";
...
const UsersTable = () => {
  ...
  const { data: registeredUsers } = useGetRegisteredUsersQuery(undefined);
  console.log(
    "🚀 ~ file: UsersTable.tsx:7 ~ UsersTable ~ registeredUsers:",
    registeredUsers
  );
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {registeredUsers?.data?.length > 0 ? (
              registeredUsers?.data?.map(
                (user: any, key: any) =>
                  user?.ownerEmail === userEmail && (
                    <tr key={key}>
              ...
              <tr>
                <td className="text-center items-center justify-center">
                  <h5 className="font-medium text-center text-gray-700 dark:text-white">
                    {`No Data Found`}
                  </h5>
                </td>
              </tr>
